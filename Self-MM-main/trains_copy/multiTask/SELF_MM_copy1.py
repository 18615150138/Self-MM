        ...
        while True:
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    loss = get_log_loss(evidences, evidence_a, Y, epochs, num_classes=num_classes,annealing_step=annealing_step, gamma=gamma, device=device)
                    # backward
                    loss.backward()
                    if self.args.grad_clip != -1.0:
                        torch.nn.utils.clip_grad_value_([param for param in model.parameters() if param.requires_grad], self.args.grad_clip)
                    train_loss += loss.item()
                    y_pred.append(evidence_a.cpu())
                    y_true.append(Y.cpu())
                    # update parameters
                    if not left_epochs:
                        # update
            ...
            logger.info(
                f"TRAIN-({self.args.model_name}) [{epochs - best_epoch}/{epochs}/{self.args.cur_time}] >> loss: {round(train_loss, 4)} {dict_to_str(train_results)}"
            )
