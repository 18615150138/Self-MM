...
class MMDataset(Dataset):
    ...
    def __normalize(self):
        # 将视觉和音频数据的维度从 (num_examples,max_len,feature_dim) -> (max_len, num_examples, feature_dim)
        self.vision = np.transpose(self.vision, (1, 0, 2))
        self.audio = np.transpose(self.audio, (1, 0, 2))
        # for visual and audio modality, we average across time 在时间维度上取平均值
        ...
        #将视觉和音频数据的维度从 (1, num_examples, feature_dim) 转换回 (num_examples, 1, feature_dim)。
        self.vision = np.transpose(self.vision, (1, 0, 2))
        self.audio = np.transpose(self.audio, (1, 0, 2))
