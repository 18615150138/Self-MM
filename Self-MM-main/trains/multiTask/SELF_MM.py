...
class SELF_MM():
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    # forward 那我应该去这里看看他怎么操作的这几个特征向量了，看看怎么用的这个序列数据
                    outputs = model(text, (audio, audio_lengths), (vision, vision_lengths))

                    # store results 后面模型评估acc 啥的要用，这里是个回归值
    ...
    #ULGM初始化，M,T,A,V四者的标签都为M的标签，然后基于此加减偏移量
